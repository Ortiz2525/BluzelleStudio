...
passport.serializeUser(function(user, done) {
  ...
  function createOAuthClient(){
    var resp;
    request.post({url:'https://api.heroku.com/oauth/clients', name: 'BluzelleDashboard', redirect_uri: 'https://example.com/auth/heroku/callback', headers: {'Accept': 'application/vnd.heroku+json; version=3', 'Content-Type': 'application/json"'} }, function optionalCallback(error, response, body) {
        if (error) {
